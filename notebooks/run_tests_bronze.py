...
TESTS RUN (4 per table, 2 tables = 8 total):
    Duplicate Check       — no duplicate rows on primary key
    Not Null Check        — key column and rcm_client_id never null
    File vs Table Count   — Bronze count == source CSV count
    Empty File Check      — Bronze table has at least 1 row

AUDIT:
    Results written to workspace.tirtho_db.test_audit_log
    with layer = "bronze" for all rows from this run.
"""
...
# ── Pre-flight: check Bronze tables and metadata table exist ─────────────────
for t in ["workspace.tirtho_db.bronze_charges",
